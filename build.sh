...
for gofile in "$SRC_DIR"/*.go; do
    if [ -f "$gofile" ]; then
        ...
        if [ -f "$SCRIPT_DIR/$progname" ] && [ ! -f "$SRC_DIR/$progname.go" ]; then
...
for progname in "${!programs[@]}"; do
    ...
    FILES=""
    if [ -f "$SRC_DIR/${progname}.go" ]; then
        FILES="$SRC_DIR/${progname}.go"
    fi
    
    for helper in "$SRC_DIR"/${progname}_*.go; do
        if [ -f "$helper" ]; then
            FILES="$FILES $helper"
        fi
    done
